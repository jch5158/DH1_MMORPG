...
:: StartClient.bat — Unreal Editor로 DH1_Client 를 -game 으로 N개 실행 (멀티 클라 / AOI 테스트)
::
:: Usage:
::   StartClient.bat              → 1개 (기본 해상도 1280x720)
::   StartClient.bat 3            → 3개 (기본 해상도 960x540, 창 위치 살짝 어긋남)
::   StartClient.bat 3 4          → 3개, 인스턴스 간 대기 4초 (기본 2초)
::
:: .env 선택 항목:
::   DH1_UE5_EDITOR           — UnrealEditor.exe 전체 경로 (미설정 시 UE_5.7 기본 경로)
::   DH1_CLIENT_RESX / RESY   — 해상도 (StartGame.bat 과 동일)
::   DH1_CLIENT_STAGGER_SEC   — 2번째 인자 대신 고정 스태거(초)
::   DH1_CLIENT_MULTI_COUNT   — 인자 없을 때 기본 실행 개수(1보다 크면 다중 기본값)
::
:: 참고: 동일 .uproject 를 동시에 여러 에디터가 붙잡으면 락/경고가 날 수 있음. 스태거를 늘리거나 한 인스턴스가 완전히 뜬 뒤 다음을 실행하세요.
...
if not "%DH1_UE5_EDITOR%"=="" set "UE5_EDITOR=%DH1_UE5_EDITOR%"
