...
set "CLIENT_CPP_H_OUT_DIR=%BASE_DIR%..\..\DH1_Client\Source\DH1_Client\Network\Protocol"
if not exist "%CPP_OUT_DIR%" mkdir "%CPP_OUT_DIR%"
if not exist "%CLIENT_CPP_H_OUT_DIR%" mkdir "%CLIENT_CPP_H_OUT_DIR%"

for %%f in ("%PROTO_DIR%\*.proto") do (
    "%PROTOC_PATH%" -I="%PROTO_DIR%" -I="%GOOGLE_INC%" --cpp_out="%CPP_OUT_DIR%" --descriptor_set_out="%PROTO_DIR%\%%~nf.desc" --include_imports "%%f"
)

xcopy /S /Y "%CPP_OUT_DIR%\*.pb.h" "%CLIENT_CPP_H_OUT_DIR%\"