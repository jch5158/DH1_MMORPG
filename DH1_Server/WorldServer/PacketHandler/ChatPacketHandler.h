...
public:
	...
	static void Init()
	{
		...
		sPacketHandleMap[packet_id::eChatPacketId::S2S_REALM_CHAT_DELIVER_NOT] = [](const uint16 size, const byte* pBuffer, const PacketSessionRef& pSession) -> bool
		{
			return HandlePacket<Protocol::S2S_REALM_CHAT_DELIVER_NOT>(size, pBuffer, pSession, HANDLE_S2S_REALM_CHAT_DELIVER_NOT);
		};
	...
	static bool HANDLE_S2S_REALM_CHAT_DELIVER_NOT(const Protocol::S2S_REALM_CHAT_DELIVER_NOT& packet, const PacketSessionRef& pSession);
	

	static NetSendBufferRef MakeSendBuffer(const Protocol::S2S_REALM_CHAT_SUBMIT_NOT& packet) { return MakeSendBuffer(packet, packet_id::eChatPacketId::S2S_REALM_CHAT_SUBMIT_NOT); }
