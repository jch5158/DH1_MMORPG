...
NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE(RedisWorldRegistration, worldId, worldName, currentPlayers, maxPlayers, status)
...
void RedisService::UpdateWorldServerInfo(const RedisWorldRegistration& info, const int64 ttlSeconds)
{
	mpActorService->GetActorDispatcher().Post(mRedisActorId, [argService = shared_from_this(), argInfo = info, argTtl = ttlSeconds]() -> void
		{
			...
			const std::string redisKey = "World:" + std::to_string(argInfo.worldId);
