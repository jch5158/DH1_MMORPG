...
public:
	...
	static void Init()
	{
		sPacketHandleMap[packet_id::eWorldPacketId::C2S_WORLD_LIST_REQ] = [](const uint16 size, const byte* pBuffer, const PacketSessionRef& pSession) -> bool
		{
			return HandlePacket<Protocol::C2S_WORLD_LIST_REQ>(size, pBuffer, pSession, HANDLE_C2S_WORLD_LIST_REQ);
		};
		sPacketHandleMap[packet_id::eWorldPacketId::C2S_WORLD_SELECT_REQ] = [](const uint16 size, const byte* pBuffer, const PacketSessionRef& pSession) -> bool
		{
			return HandlePacket<Protocol::C2S_WORLD_SELECT_REQ>(size, pBuffer, pSession, HANDLE_C2S_WORLD_SELECT_REQ);
		};
	...
	static bool HANDLE_C2S_WORLD_LIST_REQ(const Protocol::C2S_WORLD_LIST_REQ& packet, const PacketSessionRef& pSession);
static bool HANDLE_C2S_WORLD_SELECT_REQ(const Protocol::C2S_WORLD_SELECT_REQ& packet, const PacketSessionRef& pSession);


	static NetSendBufferRef MakeSendBuffer(const Protocol::S2C_WORLD_LIST_RES& packet) { return MakeSendBuffer(packet, packet_id::S2C_WORLD_LIST_RES); }
static NetSendBufferRef MakeSendBuffer(const Protocol::S2C_WORLD_SELECT_RES& packet) { return MakeSendBuffer(packet, packet_id::S2C_WORLD_SELECT_RES); }
