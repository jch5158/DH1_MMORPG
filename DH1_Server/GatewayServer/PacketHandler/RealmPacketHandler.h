...
public:
	...
	static void Init()
	{
		sPacketHandleMap[packet_id::eRealmPacketId::C2S_REALM_LIST_REQ] = [](const uint16 size, const byte* pBuffer, const PacketSessionRef& pSession) -> bool
		{
			return HandlePacket<Protocol::C2S_REALM_LIST_REQ>(size, pBuffer, pSession, HANDLE_C2S_REALM_LIST_REQ);
		};
		sPacketHandleMap[packet_id::eRealmPacketId::C2S_REALM_SELECT_REQ] = [](const uint16 size, const byte* pBuffer, const PacketSessionRef& pSession) -> bool
		{
			return HandlePacket<Protocol::C2S_REALM_SELECT_REQ>(size, pBuffer, pSession, HANDLE_C2S_REALM_SELECT_REQ);
		};
	...
	static bool HANDLE_C2S_REALM_LIST_REQ(const Protocol::C2S_REALM_LIST_REQ& packet, const PacketSessionRef& pSession);
	static bool HANDLE_C2S_REALM_SELECT_REQ(const Protocol::C2S_REALM_SELECT_REQ& packet, const PacketSessionRef& pSession);
	

	static NetSendBufferRef MakeSendBuffer(const Protocol::S2C_REALM_LIST_RES& packet) { return MakeSendBuffer(packet, packet_id::eRealmPacketId::S2C_REALM_LIST_RES); }
	static NetSendBufferRef MakeSendBuffer(const Protocol::S2C_REALM_SELECT_RES& packet) { return MakeSendBuffer(packet, packet_id::eRealmPacketId::S2C_REALM_SELECT_RES); }
