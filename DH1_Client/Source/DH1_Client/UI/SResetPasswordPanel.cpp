...
FReply SResetPasswordPanel::OnSendVerifyCodeClicked()
{
	...
	const FString Email = NormalizeEmail(EmailInputText->GetText().ToString());
	if (Email.IsEmpty())
	{
	...
	Request->SetURL(ResolveAuthEndpointUrl(TEXT("forgot-password")));
...
FReply SResetPasswordPanel::OnPasswordConfirmClicked()
{
	...
	const TSharedRef<IHttpRequest, ESPMode::ThreadSafe> Request = FHttpModule::Get().CreateRequest();
	Request->SetURL(ResolveAuthEndpointUrl(TEXT("reset-password")));
	Request->SetVerb(TEXT("POST"));
	Request->SetHeader(TEXT("Content-Type"), TEXT("application/json"));
	Request->SetContentAsString(JsonString);
